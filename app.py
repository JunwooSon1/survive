...
def about_dialog():
    ...
    with tab_overview:
        ...
        st.markdown("**결측치는 왜 문제이고, 어떻게 처리하나요?**")
        st.write(
            "빈칸이 있는 환자를 통째로 빼버리면(완전제거법) 데이터가 확 줄어들고, 하필 특정 "
            "조건의 환자들만 결측이 많다면 남은 데이터가 한쪽으로 쏠려서 결과가 왜곡될 수 있어요. "
            "그렇다고 평균값으로 채우면, 그 변수와 다른 변수들 사이의 관계(예: 종양 크기가 클수록 "
            "병기도 높은 경향)를 무시하게 돼서 예측력이 떨어져요.\n\n"
            "그래서 이 프로그램은 **Masked Autoencoder**(마스킹 오토인코더)라는 신경망을 씁니다. "
            "학습 과정에서 값이 다 채워진 환자 기록 중 일부를 일부러 가려놓고, **그 환자의 나머지 "
            "관측값들만 보고 가려진 값을 맞히도록** 반복 훈련시켰어요. 이 과정에서 신경망은 "
            "'이 환자의 다른 값들이 이러면 이 값은 대략 이 정도일 것'이라는, 변수들 사이의 실제 "
            "관계 패턴을 학습하게 됩니다. 그래서 실제 결측치가 나타났을 때도, 단순 평균이 아니라 "
            "그 환자의 다른 정보를 근거로 한 채움값을 만들어낼 수 있어요."
        )

        st.markdown("**중도절단은 왜 문제이고, 어떻게 처리하나요?**")
        st.write(
            "생존분석 데이터에는 연구가 끝날 때까지 사건(사망)이 일어나지 않은 환자들이 있어요. "
            "이 환자들은 '그 시점까지는 살아있었다'는 정보는 확실하지만, 그 이후 어떻게 됐는지는 "
            "모릅니다. 이걸 그냥 '사망 안 함(생존)'으로 취급하면 위험도를 실제보다 낮게 추정하게 "
            "되고, 아예 데이터에서 빼버리면 관찰 기간이 긴 환자일수록 데이터에서 사라지는 "
            "이상한 편향이 생겨요.\n\n"
            "그래서 Engine1(DeepHit)과 Engine2(Cox) 둘 다, 사망 여부와 별개로 **'몇 개월째까지 "
            "관찰됐는지'라는 정보 자체를 손실 계산에 직접 포함**시키는 방식으로 설계돼 있어요. "
            "중도절단된 환자도 '적어도 이 시점까지는 사건이 없었다'는 유효한 정보로 취급되기 "
            "때문에, 데이터를 버리지 않고 그대로 활용할 수 있습니다."
        )
    ...
    with tab_engine1:
        st.write(
            "**한 문장으로**: 결측치가 있는 신규 환자의 데이터를 넣으면, 빈칸을 채우는 것과 "
            "위험도를 예측하는 것을 하나의 신경망이 동시에 처리합니다."
        )
        st.write(
            "**구조**: 임상변수 16개를 Masked Autoencoder에 통과시켜 8차원 **잠재벡터**(환자의 "
            "핵심 특징을 압축한 표현)로 바꾸는 동시에 결측치를 복원합니다. 이 잠재벡터를 다시 "
            "**DeepHit**(이산시간 생존분석 신경망)에 입력해서, 지정한 시점(12/36/60/120개월)까지 "
            "질병으로 사망할 확률과 다른 원인으로 사망할 확률을 각각 계산해요. DeepHit은 시간을 "
            "여러 구간으로 나눠서, 각 구간마다 '이번 구간에 사건이 일어날 확률'을 직접 추정하는 "
            "방식이라 중도절단된 환자도 손실 함수에 자연스럽게 포함됩니다."
        )
        st.write("**성능**: 검증 데이터 기준 C-index(순위 예측 정확도) 약 0.63 (0.5=무작위 수준, 1.0=완벽 예측) — "
                 "결측치 처리와 예측을 각각 따로 최적화한 파이프라인과 대등한 수준이에요.")
        st.write(
            "**반사실적 치료효과 추정**: 같은 환자의 치료 변수만 인위적으로 0→1로 바꿔서 다시 "
            "예측시킨 뒤 그 차이를 봅니다. 다만 실제로 치료를 받고 안 받고를 가른 다른 이유(예: "
            "중증도)가 데이터에 안 잡혀있으면 이 차이가 진짜 치료효과가 아닐 수 있어서, 통계적 "
            "검정인 Engine2보다는 참고용으로 보시는 게 안전해요."
        )

    with tab_engine2:
        st.write(
            "**한 문장으로**: 특정 치료를 받은 그룹과 안 받은 그룹의 생존 데이터를 통계적으로 "
            "비교해서, 그 치료가 위험도에 유의미한 영향을 주는지 검정합니다."
        )
        st.write(
            "**구조**: **Cox 비례위험모형**을 사용합니다. 각 환자의 '위험도가 시간에 따라 얼마나 "
            "변하는지(위험함수)'를 직접 추정하는 대신, 사건이 일어난 시점들의 순서와 그 시점에 "
            "누가 아직 위험에 노출돼 있었는지(중도절단된 환자는 그 시점 이후 위험집합에서 자연스럽게 "
            "빠짐)만으로 치료변수의 효과를 추정하는 방식이에요. 화학·호르몬·방사선 치료 중 하나를 "
            "고르면, 그 치료를 받은 그룹과 안 받은 그룹의 **위험비(HR)**와 **유의확률(p-value)**을 "
            "계산해줍니다."
        )
        st.write(
